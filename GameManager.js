/**
 * AI可読性・先祖返り防止コメント:
 * 【フェーズ1: 経済連動ロジックのハブ結合 (絶対安定化対応)】
 * 1. 空路開拓確認UIにて動的コストを表示。
 * 2. 開拓実行時に動的コストで所持金をチェック・減算。
 * 3. 機体購入・売却時に Config の動的価格テーブルとリセールバリューを適用。
 * 4. 【追加】animate内のEconomyManagerの更新時に `networkManager` を渡し、
 * ネットワーク全体の総延長による収益ボーナス計算（絶対安定化）を実現しました。
 */
...
export class GameManager {
    ...
    animate() {
        ...
        // ★修正 (Proposal 017): networkManagerの参照を渡し、ネットワーク規模によるボーナス算出を行う
