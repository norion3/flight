 ...
 * 4. 【ギザ3連ピラミッド実測黄金比率アライメント ＆ 南島広大台地ジャスト収容】
...
export class MuContinentManager {
    ...
    _buildMonuments() {
        ...
        //    ★最適台地アライメント：(-0.06, +0.08) で東側海岸線マージンを3倍に確保
        // -------------------------------------------------------------------------
        const mainLayers = [
        ...
        this.pyrMainGroup.position.set(-0.06, 0.08, 0); // 南島の中央広大台地へジャスト収容
        this.pyramidGroup.add(this.pyrMainGroup);

        // -------------------------------------------------------------------------
        // ② 第2神殿：中ピラミッド（カフラー王相当 / 底面幅 0.30 / 高さ 0.28 / 5層）
        //    ★ギザ実測黄金間隔アライメント：(-0.16, -0.26)
        // -------------------------------------------------------------------------
        const midLayers = [
        ...
        this.pyrMidGroup.position.set(-0.16, -0.26, 0); // 南西台地へ黄金間隔で展開
        this.pyramidGroup.add(this.pyrMidGroup);

        // -------------------------------------------------------------------------
        // ③ 第3神殿：小ピラミッド（メンカウラー王相当 / 底面幅 0.20 / 高さ 0.18 / 3層）
        //    ★ギザ実測オフセット＆南西尾根配置：(-0.22, -0.48)
        // -------------------------------------------------------------------------
        const smallLayers = [
        ...
        this.pyrSmallGroup.position.set(-0.22, -0.48, 0); // 南西端の広大な台地を満たす実測比率オフセット配置
